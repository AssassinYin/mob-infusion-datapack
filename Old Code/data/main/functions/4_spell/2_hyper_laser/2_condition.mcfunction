...
#Timer
execute if score @s SkillCharging matches -1 if score @s 2_Timer matches 1.. run tellraw @s ["",{"text":"Cannot use until ","color":"gold"},{"score":{"name":"@s","objective":"DisplayTimer"},"color":"gold"},{"text":" second(s) later","color":"gold"}]

execute if score @s SpellLocked matches 0 if score @s SkillCharging matches -1 unless score @s StunTimer matches 1.. unless score @s AirborneTimer matches 1.. unless score @s SilenceTimer matches 1.. if score @s Originality >= @s BaseAbilityON if score @s 2_Timer matches 0 run function main:4_spell/2_hyper_laser/3_succeed

execute if score @s SkillCharge = @s SpellItemID if score @s SkillCharging matches 1..40 if predicate main:sneaking at @s positioned ~ ~1.25 ~ run function main:4_spell/2_hyper_laser/4_cast
execute if score @s SkillCharge = @s SpellItemID if score @s SkillCharging matches 1..40 unless predicate main:sneaking at @s positioned ~ ~1.55 ~ run function main:4_spell/2_hyper_laser/4_cast
