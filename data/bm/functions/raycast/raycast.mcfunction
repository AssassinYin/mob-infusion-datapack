...
execute positioned ~-0.01 ~-0.01 ~-0.01 as @e[tag=!raycaster,tag=!this,tag=!hit,dx=0] if score #bool raycast matches 1 run function bm:raycast/hit

execute if score %penetrateThroughWall raycast matches 0 positioned ^ ^ ^0.25 run function bm:raycast/move
execute unless score %penetrateThroughWall raycast matches 0 positioned ^ ^ ^0.25 run function bm:raycast/raycast